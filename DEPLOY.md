...
## 2. Build settings — none required

The repo ships `functions/[[path]].js` as a committed, fully self-contained
bundle (Hono and all app code inlined, regenerated by `npm run build`). Pages
can therefore deploy it **with no build command at all** — when Pages logs
`No build command specified. Skipping build step.`, the deploy still succeeds
because there is nothing left to resolve.

Optionally, set **Settings → Build → Build command** to `npm run build`
(output directory `public`) so the bundle is regenerated fresh on every push
instead of relying on the committed copy. Either way works; the committed
bundle is the safety net, and `npm test` fails if it drifts from the source.
...
npm test           # 78-check end-to-end suite
```
...
**`Could not resolve "hono"`** — the committed Functions bundle is stale or was
edited by hand. Run `npm run build` and commit the regenerated
`functions/[[path]].js` (never edit it directly; the source is
`src/pages-entry.js`).
