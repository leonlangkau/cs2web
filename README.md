...
> The Pages Git integration needs **no build command**: `functions/[[path]].js`
> is a committed, self-contained bundle (regenerated by `npm run build`), so
> deploys succeed even when Pages skips `npm install`.
...
```
src/pages-entry.js      Cloudflare Pages entry source
functions/[[path]].js   generated self-contained Pages bundle (committed)
...
- `functions/[[path]].js` from `src/pages-entry.js` — the self-contained Pages
  bundle (esbuild is version-pinned so the output is byte-stable)
