/**
 * React Bits FaultyTerminal (OGL) — patched: the render loop pauses while the
 * canvas is off-screen or the tab is hidden, the default dpr is low (the
 * shader is expensive and the look is blocky by design), and the WebGL
 * context is released on unmount. The caller only mounts it while the hero
 * is on screen, so one context lives at a time.
 */
...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    let renderer;
    try { renderer = new Renderer({ dpr }); } catch { return undefined; }
    ...
    const update = (t) => {
      ...
      if (pageLoadAnimation && loadStart > 0) program.uniforms.uPageLoadProgress.value = Math.min((t - loadStart) / 2000, 1);
      if (mouseReact) {
  ...
  }, [dpr, timeScale, scale, gridMul, digitSize, scanlineIntensity, glitchAmount, flickerAmount, noiseAmp, chromaticAberration, dither, curvature, tintVec, mouseReact, mouseStrength, pageLoadAnimation, brightness, handleMouseMove]);

  return <div ref={containerRef} className={`faulty-terminal-container ${className}`} aria-hidden="true" />;
}
